    ...
    def htmlAnalysis(self, data):
        ...
        try:
            ...
            for j in range(len(many)):
                for k in range(len(self.list)):  # 之前的数据
                    if self.list[k][1] == many[j][1]:
                        print("去除重复数据 {}".format(many[j][1]))
                        tmp.append(j)
            tmp = list(set(tmp))  # 列表去重
            self.list = many
            tmp.reverse()  # 反转字符 防止删除越界
            # 移除重复数据
            for x, item in enumerate(tmp):
                many.remove(many[item])
            ...
            self.cur.executemany("INSERT INTO " + self.tableName + "  VALUES (?,?,?,?,?,?,?,?,?,?,?)", many)

            # print("INSERT INTO bagumi_offical VALUES {}".format(value))
...
"""
    ...
    ## 55 个对象
    # s.mian(bili[54][2], "bilibili.db", create_table.format(bili[54][0]), bili[54][0],bili[54][1])


    for i in range(len(bili)):
        r = requests.get(bili[i][1].format("1", "2016-5-24"))
        bili[i][2] = int(re.findall('<span class="result custom-right-inner">共 (\d+) 页', str(r.text))[0])
        print(bili[i][2])
    print("列表数据更新完毕")
    threadNum = len(bili)
    i = 0  # i 用来计算执行的线程数
    while (i < threadNum):
        if i < threadNum:
            thread1 = Th("线程一 ", bili[i][2], create_table.format(bili[i][0]), bili[i][0], bili[i][1])
            thread1.start()
            i += 1
        if i < threadNum:
            thread2 = Th("线程二 ", bili[i][2], create_table.format(bili[i][0]), bili[i][0], bili[i][1])
            thread2.start()
            i += 1
        if i < threadNum:
            thread3 = Th("线程三 ", bili[i][2], create_table.format(bili[i][0]), bili[i][0], bili[i][1])
            thread3.start()
            i += 1
        if i < threadNum:
            thread4 = Th("线程四 ", bili[i][2], create_table.format(bili[i][0]), bili[i][0], bili[i][1])
            thread4.start()
            i += 1
        if i < threadNum:
            thread5 = Th("线程五 ", bili[i][2], create_table.format(bili[i][0]), bili[i][0], bili[i][1])
            thread5.start()
            i += 1
        # 进程等待
        thread1.join()
